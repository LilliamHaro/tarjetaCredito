function isValidCard(){
  ...
  var newArr=[]; //para contener los número de posiciones impares multiplicado y lode demas
  ...
  for(var j=0;j<array.length;j++){ //multiplicamos los numeros en posiciones impares contando desde 0
    if(j%2===1){
      ...
      if(multipli>=10){
        ...
        var newNum=div+resi;
        newArr.push(newNum); //almacenamos el numero en newArray
      }
      else{
  ...
  for(var k=0;k<newArr.length;k++){//recorremos el arreglo para obtener la suma de sus elemntos
    sum+=newArr[k];
  }

  if(sum%10===0){  //verificamos si es divisible netre 10 mostrar que la tarjeta es valida
